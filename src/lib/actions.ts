...
import { AssignmentSchema, AttendanceSchema, ClassSchema, ExamSchema, LessonSchema, ParentSchema, ResultSchema, StudentSchema, SubjectSchema, TeacherSchema } from "./datasource"
...
// Assignment
export const createUpdateAssignment = async (currentState: { success: boolean, error: boolean }, data: AssignmentSchema) => {
    try {
        if (data.id) {
...
// Result
export const createUpdateResult = async (currentState: { success: boolean, error: boolean }, data: ResultSchema) => {
    try {
        if (data.id) {
            ...
            await prisma.result.create({
                data: {
                    ...
                    score: data.score,
                    examId: data.examId === 0 ? null : data.examId
                }
            })
...
// Attendance
export const createUpdateAttendance = async (currentState: { success: boolean, error: boolean }, data: AttendanceSchema) => {
    try {
        if (data.id) {
...
export const deleteAttendance = async (currentState: { success: boolean, error: boolean }, data: FormData) => {
    const id = data.get("id") as string
    try {
